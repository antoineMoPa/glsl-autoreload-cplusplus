...
FrameBuffer ShaderGif::ShaderGif::fb;

// The data of the render-to-texture quad
GLuint ShaderGif::quad_vertexbuffer;
...
*/
void ShaderGif::resize(int rhs_w, int rhs_h) {
	w = rhs_w;
	h = rhs_h;
...
void ShaderGif::load_default_shaders() {
	// default shaders
	char * vertex_path =
		strdup("./vertex.glsl");
	char * frag_path =
...
*/
float ShaderGif::get_frame_time() {
	if(curr_time < 0.0) {
...
void ShaderGif::post_process_render(int pass) {
	glViewport(0, 0, w, h);
...
*/
void ShaderGif::create_render_quad() {
...
*/
float ShaderGif::get_positive_numeric_arg(const char * name) {
...
*/
void ShaderGif::init_watch_files() {
